...
public class GalleryClient {
  ...
  */
  public void FindApps(String keywords, int start, int count, int sortOrder, final boolean refreshable) {
    ...
    MESSAGES.gallerySearchError()) {
      @Override
      public void onSuccess(Method method, GalleryAppListResult appsResult) {
    ...
    ode.getGalleryService().findApps(new FindAppsRequest(keywords, start,count),callback);
  }
 /**
  ...
  */
  public void GetAppsByDeveloper(int start, int count, String developerId) {
    ...
    // This is below the call back, but of course it is done first
    ode.getGalleryService().getDeveloperApps(new DeveloperAppsRequest(developerId, start,count), callback);
  }
 /**
  ...
  */
  public void GetFeatured(int start, int count, int sortOrder, final boolean refreshable) {
    ...
    // This is below the call back, but of course it is done first
    ode.getGalleryService().getFeaturedApp(new PagedRequest(start, count), callback);
  }
/**
  ...
  */
  public void GetTutorial(int start, int count, int sortOrder, final boolean refreshable) {
    ...
    // This is below the call back, but of course it is done first
    ode.getGalleryService().getTutorialApp(new PagedRequest(start, count), callback);
  }
/**
  ...
  */
  public void GetMostRecent(int start, int count, final boolean refreshable) {
    ...
    // This is below the call back, but of course it is done first
    ode.getGalleryService().getRecentApps(new PagedRequest(start, count), callback);
  }
  /**
  ...
  */
  public void GetMostLiked(int start, int count, final boolean refreshable) {
    ...
    ode.getGalleryService().getMostLikedApps(new PagedRequest(start,count), callback);
  }
  /**
  ...
  */
  public void GetMostDownloaded(int start, int count, final boolean refreshable) {
    ...
    // ok, this is below the call back, but of course it is done first
    ode.getGalleryService().getMostDownloadedApps(new PagedRequest(start,count),callback);
  }
  /**
  ...
  */
  public void GetComments(long appId,int start,int count) {
    ...
    MESSAGES.galleryCommentError()) {
      @Override
      public void onSuccess(Method method, List<GalleryComment> comments) {
  ...
  */
  public boolean loadSourceFile(GalleryApp gApp, String newProjectName, final PopupPanel popup) {
    ...
    MESSAGES.createProjectError()) {
      @Override
      public void onSuccess(Method method, UserProject projectInfo) {
    ...
    ode.getProjectService().newProjectFromGallery(new NewProjectFromGalleryRequest(projectName, sourceKey, galleryId), callback);
    return true;
  }
  ...
  */
  public void appWasDownloaded(final long galleryId, final String userId) {
    ...
    final Ode ode = Ode.getInstance();
    final OdeAsyncCallback<Void> callback = new OdeAsyncCallback<Void>(MESSAGES.galleryDownloadedAppsError()) {
      @Override
      public void onSuccess(Method method, Void result) {
        ...
        final OdeAsyncCallback<GalleryApp> appCallback = new OdeAsyncCallback<GalleryApp>(MESSAGES.galleryError()) {
          @Override
        ...
        final OdeAsyncCallback<Boolean> checkCallback = new OdeAsyncCallback<Boolean>(MESSAGES.galleryError()) {
          @Override
          public void onSuccess(Method method, Boolean b) {
            //email will be send automatically if condition matches (in ObjectifyGalleryStorageIo)
          }
        };
        Ode.getInstance().getGalleryService().checkIfSendAppStats(galleryId, new CheckIfSendAppStatsRequest(userId,
