...
import com.google.appinventor.shared.rpc.component.DeleteImportedComponentRequest;
import com.google.appinventor.shared.rpc.component.RenameImportedComponentRequest;
...
public class ComponentRenameWizard extends Wizard{
    ...
    protected ComponentRenameWizard(final String defaultTypeName, long projectId, final List<ProjectNode> compNodes) {
        ...
        initFinishCommand(new Command() {
            ...
            public void execute() {
                ...
                if (TextValidators.checkNewComponentName(newName)) {
                    ode.getComponentService().renameImportedComponent(
                            new RenameImportedComponentRequest(defaultTypeName, newName, destinationProjectId),
                            new RenameComponentCallback());
        ...
        initCancelCommand(new Command() {
            @Override
            public void execute() {
                ode.getComponentService().deleteImportedComponent(
                        new DeleteImportedComponentRequest(defaultTypeName, destinationProjectId), new MethodCallback<Void>() {
                    @Override
