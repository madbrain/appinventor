...
public class MockMarker extends MockMapFeatureBaseWithFill {
  ...
  private void setImageAsset(String assetPath) {
    ...
    if (assetPath.endsWith(".svg")) {
      Ode.getInstance().getProjectService().load(Collections.singletonList(descriptor), new MethodCallback<List<FileDescriptorWithContent>>() {
        @Override
        public void onFailure(Method method, Throwable arg0) {
          GWT.log("Error occurred loading image asset", arg0);
        }

        @Override
        public void onSuccess(Method method, List<FileDescriptorWithContent> arg0) {
