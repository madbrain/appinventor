 ...
 *      the exception passed to {@link #onFailure(Method,Throwable)} is
 *      displayed using {@link ErrorReporter#reportError(String)}.
 * </ol>
 *
 * @param <T> type of object returned by successful RPC call
 */
public abstract class OdeAsyncCallback<T> implements MethodCallback<T> {

  private String failureMessage;

  /**
   * Constructor for when caller wants subclass to display the message in
   * the {@link Throwable} passed to {@link #onFailure(Method,Throwable)}, rather
   ...
   * displayed by {@link #onFailure(Method,Throwable)}
   *
