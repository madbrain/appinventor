...
import com.google.appinventor.client.explorer.youngandroid.*;
...
public class Ode implements EntryPoint {
    ...
    private final ProjectServiceAsync projectService = GWT.create(ProjectServiceAsync.class);

    // Web service for gallery related information
    private final GalleryServiceAsync galleryService = GWT.create(GalleryServiceAsync.class);

    // Web service for user related information
    private final UserInfoServiceAsync userInfoService = GWT.create(UserInfoServiceAsync.class);

    // Web service for get motd information
    private final GetMotdServiceAsync getMotdService = GWT.create(GetMotdServiceAsync.class);

    // Web service for component related operations
    private final ComponentServiceAsync componentService = GWT.create(ComponentServiceAsync.class);
    private final AdminInfoServiceAsync adminInfoService = GWT.create(AdminInfoServiceAsync.class);

    //Web service for CloudDB authentication operations
    private final CloudDBAuthServiceAsync cloudDBAuthService = GWT.create(CloudDBAuthServiceAsync.class);
    ...
    public void loadGallerySettings() {
        ...
        final OdeAsyncCallback<GallerySettings> callback = new OdeAsyncCallback<GallerySettings>(MESSAGES.gallerySettingsError()) {
            @Override
            public void onSuccess(Method method, GallerySettings settings) {
        ...
        if (templateLoadingFlag) {  // We are loading a template, open it instead
            ...
            try {
                ...
                final OdeAsyncCallback<GalleryApp> callback = new OdeAsyncCallback<GalleryApp>(
                    ...
                    @Override
                    public void onSuccess(Method method, GalleryApp app) {
                        if (app == null) {
                            openProject(value);
    ...
    public void onModuleLoad() {
        ...
        OdeAsyncCallback<Config> callback = new OdeAsyncCallback<Config>(MESSAGES.serverUnavailable()) {

            @Override
            public void onSuccess(Method method, Config result) {
                config = result;
                user = result.getUser();
                ...
                if (noop > 0) {
                    ...
                    Timer t = new Timer() {
                        @Override
                        public void run() {
                            userInfoService.noop(new MethodCallback<Void>() {
                                @Override
                                public void onSuccess(Method m, Void e) {
                                }

                                @Override
                                public void onFailure(Method m, Throwable e) {
                                }
                            });
                        }
                    };
                        ...
                        projectManager.addProjectManagerEventListener(new ProjectManagerEventAdapter() {
                            ...
                            public void onProjectsLoaded() {
                                ...
                                OdeAsyncCallback<String> templateCallback =
                                        new OdeAsyncCallback<String>(MESSAGES.createProjectError()) {
                                            @Override
                                            public void onSuccess(Method method, String json) {
...
//    mainPanel.add(switchToBlocksButton, DockPanel.EAST);
    ...
    private void setupMotd() {
        MethodCallback<Integer> callback = new MethodCallback<Integer>() {
            ...
            @Override
            public void onSuccess(Method method, Integer intervalSecs) {
                if (intervalSecs > 0) {
                    topPanel.showMotd();
    ...
    public void blocksTruncatedDialog(final long projectId, final String fileId, final String content, final OdeAsyncCallback callback) {
        ...
        continueSession.addClickListener(new ClickListener() {
            public void onClick(Widget sender) {
                ...
                getProjectService().save2(new Save2Request(getSessionId(), projectId, fileId, true, content), callback);
            }
        });
        holder.add(continueSession);
    ...
    public void recordCorruptProject(long projectId, String fileId, String message) {
        getProjectService().recordCorruption(new RecordCorruptionRequest(projectId, fileId, message),
    ...
    public void screenShotMaybe(final Runnable next, final boolean deferred) {
        ...
        currentFileEditor.getBlocksImage(new Callback<String, String>() {
            ...
            public void onSuccess(String result) {
                ...
                projectService.screenshot(new ScreenshotRequest(sessionId, projectId, screenShotName, result),
                        new OdeAsyncCallback<RpcResult>() {
                            @Override
                            public void onSuccess(Method method, RpcResult result) {
                                if (deferred) {
                                    next.run();
                                }
                            }
    ...
    private void loadBackpack() {
        userInfoService.getUserBackpack(new MethodCallback<String>() {
            @Override
            public void onSuccess(Method method, String backpack) {
